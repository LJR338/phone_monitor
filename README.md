...
小米 HyperOS 设备在长期使用后稳定性逐步下降，表现为电池衰减、CPU 调度漂移、后台耗电异常等问题。Android 原生设置面板仅提供瞬时快照，缺乏**长周期趋势追踪**能力。本项目旨在填补这一空白：通过 ADB 协议在非 root 环境下零侵入采集数据，提供开发者级的可视化运维面板。

### 目标设备

| 项目 | 详情 |
|------|------|
| 设备型号 | 小米 10 Ultra（代号 cas） |
| 系统版本 | Android 13 + HyperOS V816 |
| Root 状态 | 未 root |
| SELinux | Enforcing |
| ro.debuggable | 0 |
...
| # | 痛点 | 本项目方案 |
|---|------|-----------|
| 1 | **电池老化不可感知**——系统仅显示当前电量百分比，无法追踪满充容量的长期衰减趋势 | 每小时采集 `charge_full` 数据并持久化为 JSON，线性回归预测衰减至 80% 的剩余天数 |
| 2 | **CPU 调度不透明**——三丛集架构（大核/中核/小核）的实时频率和 governor 策略对用户完全不可见 | 可视化每核心实时频率柱状图 + 调度策略只读监控表，支持 HyperOS 性能模式一键切换 |
| 3 | **耗电设置频繁被回滚**——OTA 升级或重启后 WiFi 扫描、后台同步等耗电设置被系统自动恢复 | 13 项 Settings 键（global/secure/system 三个 Namespace）的可视化开关面板 +「一键省电」批量写入 |
| 4 | **App 功耗无明细**——系统电池统计仅提供笼统的百分比，无法按 UID 拆解各应用的 CPU / 网络 / 唤醒锁耗电 | 按 UID 统计 CPU 使用率、Wakelock 持有时间、网络收发流量，结合 meminfo 明细做全维度分析 |
| 5 | **网络质量无法量化**——WiFi/移动网络延迟、抖动、丢包情况难以追踪，间歇性网络问题几乎无法复现 | （规划中）定期测速并记录 RTT / Jitter / Loss 趋势，辅助定位网络相关的耗电或卡顿问题 |

### 项目定位

面向 **开发者 / 进阶用户** 的本地化 Android 设备运维面板。核心原则：

- **零侵入**：仅通过 ADB 协议读取 `dumpsys`、`sysfs`、`/proc` 等标准接口，不安装任何 APK，不修改系统分区
- **单文件部署**：`phone_monitor.py` 一个文件包含完整后端 + 内嵌前端 HTML，无外部依赖
- **本地优先**：所有数据存储在本地 JSON 文件，不上传云端
